...
class FloorplanModel(nn.Module):
    def __init__(self, text_input_dim=128, image_input_dim=(1, 128, 128), hidden_dim=256, output_dim=128):
        ...
        self.image_cnn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(2),  # Downsample by 2 -> (batch_size, 16, 64, 64)
            nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1),  # Second convolution
            nn.ReLU(),
            nn.MaxPool2d(2),  # Downsample by 2 -> (batch_size, 32, 32, 32)
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),  # Third convolution
            nn.ReLU(),
            nn.MaxPool2d(2),  # Downsample by 2 -> (batch_size, 64, 16, 16)
            nn.Flatten(),  # Flatten the feature maps
            nn.Linear(64 * 16 * 16, hidden_dim),  # Fully connected layer
            nn.ReLU()
        )
